...
export function resolveChartColor(value) {
...
export function getAccentCssVar() {
...
export const CHART_SUPPORT_COLORS = [
  'var(--pa-web)',
  'var(--pa-edu)',
  'var(--pa-pink)',
  'var(--pa-purple)',
...
export function assignSegmentColors(segments) {
...
export function assignSeriesColors(seriesList) {
...
export function getChartColorPalette(count = 5, { accentFirst = true } = {}) {
  const colors = [];
...
export function getAccentPalette(count = 5) {
...
function resolveChartHeight(mountEl, fallback = 230) {
  ...
  const measured = mountEl.clientHeight
    || parseFloat(getComputedStyle(mountEl).height)
    || 0;
...
export function destroyChart(mountEl) {
  if (!mountEl) return;
...
export function renderDonutChart(mountEl, segments) {
  ...
  const rings = hasData
    ? donutSegmentCircles(segments, size, strokeWidth)
    : `<circle cx="${size / 2}" cy="${size / 2}" r="${(size - strokeWidth) / 2 - 1}" fill="none" stroke="${resolveChartColor('var(--pa-border-soft)')}" stroke-width="${strokeWidth}" opacity="0.55"></circle>`;
...
export function renderProjGaugeChart(mountEl, pct, fillColor = 'var(--pa-green)') {
  if (!mountEl) return null;
  destroyChart(mountEl);
...
export function renderGaugeChart(mountEl, pct, fillColor = 'var(--pa-green)', options = {}) {
  if (!mountEl) return null;
  const value = Math.max(0, Math.min(100, pct));
  const colors = themeColors();
  const {
...
export function renderAreaChart(mountEl, points, color = 'var(--pa-green)') {
  if (!mountEl) return null;
...
export function renderMultiLineChart(mountEl, seriesDefs) {
  if (!mountEl) return null;
  const safeDefs = (seriesDefs || []).length
    ? seriesDefs
    : [{ name: 'Count', color: 'var(--pa-green)', points: [{ label: '-', y: 0 }] }];
