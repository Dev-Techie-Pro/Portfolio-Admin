...
export function getMediaKind(item) {
  ...
  if (
    ...
    || type.includes('text/')
    || /\.(pdf|doc|docx|txt|xls|xlsx|ppt|pptx)$/i.test(name)
  ) return 'document';
  return 'other';
}
...
function renderListMeta(item) {
  ...
  const uploaded = formatDate(item.uploadedAt);
  const size = formatFileSize(item.size, item.url);
  const usage = item.usageCount > 0 ? `Used ×${item.usageCount}` : 'Unused';
...
export function renderPaMediaCard(item, opts = {}) {
  const {
    selectCheckbox = '',
    cardClass = '',
    animationDelay = 0,
  ...
  const featuredBadge = isFeatured(item)
    ? '<span class="pa-media-card__featured"><i class="ri-star-fill"></i> FEATURED</span>'
    : '';
  const durationBadge = getMediaKind(item) === 'video'
    ? `<span class="pa-media-card__duration">${videoDuration(item)}</span>`
    : '';
  ...
  const altHtml = item.alt
    ? escapeHtml(item.alt)
    : '<em class="pa-media-card__desc-empty">No alt text set</em>';
