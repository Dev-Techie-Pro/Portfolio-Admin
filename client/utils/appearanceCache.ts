...
export const APPEARANCE_CACHE_KEY = 'pa_appearance_settings_v2';
...
export const ICON_PREVIEW_SIZES = { small: '12px', medium: '16px', large: '20px' };
...
/** Base design tokens at 14px — scaled proportionally for all font-size presets. */
...
/** Apply a proportional type scale to all --pa-fs-* tokens from a base size. */
export function applyFontSizeTokens(root: HTMLElement, fontSize: string) {
  const basePx = parseFloat(FONT_SIZE_MAP[fontSize] || FONT_SIZE_MAP['14px']);
  const scale = basePx / FONT_SIZE_TOKEN_BASE.md;
  (Object.entries(FONT_SIZE_TOKEN_BASE) as [string, number][]).forEach(([token, px]) => {
...
/** Map a base weight to the full --pa-fw-* token ladder. */
export function applyFontWeightTokens(root: HTMLElement, fontWeight: string) {
  const base = parseInt(FONT_WEIGHT_MAP[fontWeight] || FONT_WEIGHT_MAP['400'], 10);
  (Object.entries(FONT_WEIGHT_OFFSETS) as [string, number][]).forEach(([token, offset]) => {
...
export const VALID_RADII = Object.keys(RADIUS_MAP);
export const VALID_FONT_SIZES = Object.keys(FONT_SIZE_MAP);

/** Map legacy / invalid stored sizes to the nearest supported preset. */
export function migrateFontSize(size) {
...
export const VALID_FONT_WEIGHTS = Object.keys(FONT_WEIGHT_MAP);
export const VALID_SPACINGS = Object.keys(SPACING_MAP);
export const VALID_ICON_SIZES = Object.keys(ICON_SIZE_PRESETS);

export function applyIconSizeVariables(root, iconSize) {
...
/** Sync brand accent tokens (--pa-orange* / --pa-accent*) from a hex colour. */
export function applyAccentCssVariables(root, accent) {
...
export function isValidFontFamilyId(id, customFonts = []) {
  return ALL_FONT_IDS.includes(id) || customFonts.some((f) => f.id === id);
}

export function getFontGroups(customFonts = []) {
...
export function getFontById(id, customFonts = []) {
...
export function writeAppearanceCache(settings) {
...
export function applyAppearanceSettings(settings, options = {}) {
...
/** Apply cached settings synchronously as early as possible. */
export function bootstrapAppearanceFromCache() {
