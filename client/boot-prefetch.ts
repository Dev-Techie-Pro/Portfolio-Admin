...
(function bootPrefetch() {
  ...
  var resolvePage = config.resolvePage || function () { return 'dashboard'; };
  ...
  function applyAppearanceFromLocalStorage() {
    try {
      ...
      if (document.body) applyBodyStyles();
      else document.addEventListener('DOMContentLoaded', applyBodyStyles, { once: true });
