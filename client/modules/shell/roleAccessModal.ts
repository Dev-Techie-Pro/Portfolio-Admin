...
function renderList(items, variant) {
  return items.map((item) => (
    `<li class="pa-role-access-item pa-role-access-item--${variant}">
...
/**
 * Show the role access briefing once per user + role (editor / viewer only).
 * @param {{ id?: string, role?: string } | null} [profile]
 */
export async function maybeShowRoleAccessModal(profile = null) {
...
export function initRoleAccessModal() {
  ensureModalInDom();
  bindModalEvents();
}
