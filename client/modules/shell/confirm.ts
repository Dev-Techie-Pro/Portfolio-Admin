 ...
 * @param {string} [opts.confirmLabel] Temporarily replaces the confirm button label.
 * @param {string} [opts.iconClass] Remix icon class for the dialog icon.
 * @param {boolean} [opts.danger=true] When true, styles the confirm button as destructive.
 * @param {'danger'|'warning'} [opts.iconTone='danger'] Icon accent color.
 * @param {() => void|Promise<void>} opts.onConfirm Called once the user confirms.
 */
export function requestBulkAction({
  title,
  message,
  onConfirm,
  confirmLabel = 'Delete',
...
/** Alias for {@link requestBulkAction} with clearer naming for non-delete confirmations. */
...
export function requestLogout(onConfirm) {
