...
export class MediaModule extends Module {
  ...
  renderStagedGrid() {
    ...
    grid.innerHTML = this.stagedFiles
      .map((sf, i) => `<div class="pa-gallery-thumb"><img src="${escapeHtml(sf.previewUrl || sf.url || '')}" alt="${escapeHtml(sf.name)}" /><div class="pa-gallery-thumb-remove" data-i="${i}" role="button" aria-label="Remove ${escapeHtml(sf.name)}"><i class="ri-close-line"></i></div></div>`)
      .join('');
