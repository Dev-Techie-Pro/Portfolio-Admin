...
/** @deprecated Prefer live tool_categories; kept for dashboard fallbacks */
...
export class TechnologiesModule extends CrudCardModule {
  ...
  _populateCategoryFilter() {
    ...
    select.innerHTML = '<option value="all">All categories</option>'
      + sortByNewestFirst(this.categories)
        .map((c) => `<option value="${escapeHtml(String(c.id))}">${escapeHtml(c.label)}</option>`)
        .join('');
    select.value = current;
  }

  _populateCategorySelects() {
    const options = sortByNewestFirst(this.categories)
      .map((c) => `<option value="${escapeHtml(String(c.id))}">${escapeHtml(c.label)}</option>`)
      .join('');
  ...
  populateEditForm(t) {
    ...
    if (editGroup) {
      const hasCategory = t.categoryId != null
        && this.categories.some((c) => String(c.id) === String(t.categoryId));
      if (hasCategory) {
  ...
  validateForm(prefix) {
    ...
    if (!name) { this._err(`${p}Name`, true); valid = false; }
    ...
    if (!level) { this._err(`${p}Level`, true); valid = false; } else this._err(`${p}Level`, false);

    const url = ($id(`${p}Url`)?.value || '').trim();
    if (url && !isValidUrl(url)) { this._err(`${p}Url`, true); valid = false; } else this._err(`${p}Url`, false);
    ...
    return {
      ...
      sortOrder: parseSortInput($id(`${p}Sort`)?.value || '', undefined),
    };
  }
