...
export class ToolsModule extends CrudCardModule {
  constructor() {
    super({
      name: 'Tools',
      storageKey: 'pa_tools',
      ...
      menuActions: {
        'copy-name': function copyName(id) { this.copyToolName(id); },
      },
    });
    this.categories = [];
  }
  ...
  copyToolName(id) {
    ...
    navigator.clipboard?.writeText(record.name).then(
      () => this.toast('Name copied to clipboard', 'success', 2000),
  ...
  validateForm(prefix) {
    ...
    if (!categoryId) { this._err(`${p}Category`, true); valid = false; }
