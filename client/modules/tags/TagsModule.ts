...
import { closePanels, openPanel, activateTab } from '../../modules/shell/panels.js';
...
export class TagsModule extends CrudCardModule {
  constructor() {
    super({
      name: 'Tags',
      ...
      menuActions: {
        'copy-tag': function copyTag(id) { this.copyTagName(id); },
      },
  ...
  matchesSearch(record, query) {
    ...
    return String(record.tag || '').toLowerCase().includes(q)
      || String(record.projectTitle || '').toLowerCase().includes(q)
      || String(record.projectLegacyId || '').includes(q);
  }
  ...
  copyTagName(id) {
    ...
    navigator.clipboard?.writeText(record.tag).then(
      () => this.toast(`Copied “${record.tag}” to clipboard.`, 'success', 2000),
      () => this.toast('Clipboard not available.', 'danger'),
    );
  }

  navigateToProject(legacyId) {
    try { sessionStorage.setItem('pa_projects_focus_id', String(legacyId)); } catch { /* ignore */ }
  ...
  populateProjectSelects() {
    const options = this.projects
      ...
      .map((p) => `<option value="${escapeHtml(String(p.id))}">${escapeHtml(p.title || `Project #${p.id}`)}</option>`)
      .join('');
  ...
  renderGroupedCards(pageItems) {
    ...
    // Full filtered counts per project (for heading badge), not just this page slice
  ...
  renderStats() {
    ...
    const avg = projectsWithTags.size
      ? Math.round((records.length / projectsWithTags.size) * 10) / 10
      : 0;
    ...
    if (grid) {
      ...
      if (pageItems.length === 0) {
        const hasFilters = !!(this.store.get('searchQuery') || '').trim()
          || Object.values(this.store.get('filters')).some((v) => v && v !== 'all');
  ...
  validateForm(prefix) {
    ...
    const duplicate = this.store.get('records').some((r) => (
      String(r.id) !== String(editingId)
      && String(r.projectLegacyId) === projectLegacyId
      && String(r.tag).toLowerCase() === tag.toLowerCase()
    ));
    if (duplicate) {
      this._setFieldError(`${p}Name`, false);
