...
export class RecentActivitiesModule extends Module {
  constructor() {
    super({
  ...
  computeNow() {
    ...
    if (!activities.length) { this.actNow = Date.now(); return; }
    const latest = Math.max(...activities.map((a) => new Date(a.createdAt).getTime()));
    this.actNow = latest + 3600000;
  }
  ...
  getFiltered() {
    const {
      activities, typeFilter, dateFilter, sortBy, userFilter, statusFilter, searchQuery,
    } = this.store._raw;
  ...
  populateUserFilter() {
    ...
    select.innerHTML = `<option value="all">All Users</option>${users.map((u) =>
      `<option value="${escapeHtml(u.id)}"${u.id === current ? ' selected' : ''}>${escapeHtml(u.name)}</option>`,
    ).join('')}`;
  }
  ...
  renderRow(a) {
    ...
    const checkboxCell = bulkMode
      ? `<td style="width:36px;"><input type="checkbox" class="pa-msg-bulk-checkbox" id="paActBulkCb-${escapeHtml(a.id)}" data-select-id="${escapeHtml(a.id)}" ${bulkSelected ? 'checked' : ''} aria-label="Select activity ${escapeHtml(a.actionTitle)} for bulk actions" /></td>`
      : '';
  ...
  getEmptyStateHtml() {
    const hasFilters = this.store.get('searchQuery').trim()
      || this.store.get('typeFilter') !== 'all'
      || this.store.get('dateFilter') !== '7'
      || this.store.get('userFilter') !== 'all'
      || this.store.get('statusFilter') !== 'all';
  ...
  renderPagination(totalItems, totalPages, page) {
    ...
    if (prev && !prev.classList.contains('disabled')) {
      prev.addEventListener('click', () => { this.store.set('page', page - 1); this.renderTable(); });
    }
    if (next && !next.classList.contains('disabled')) {
      next.addEventListener('click', () => { this.store.set('page', page + 1); this.renderTable(); });
    }
  }
  ...
  copyActivity(a) {
    ...
    navigator.clipboard.writeText(text).then(() => {
      this.toast('Activity details copied to clipboard.', 'success', 2000);
    }).catch(() => {
  ...
  async deleteActivity(id) {
    ...
    try {
      this.statusToast('Deleting activity…', 'info', 120000);
  ...
  async bulkDelete(ids) {
    ...
    try {
      this.statusToast('Deleting activities…', 'info', 120000);
  ...
  bindEvents() {
    ...
    if (refreshBtn) {
      this.on(refreshBtn, 'click', async () => {
        ...
        this.toast('Activities refreshed.', 'success', 2000);
      });
    }
