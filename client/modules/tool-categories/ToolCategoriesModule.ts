...
import { DEFAULT_CATEGORY_ICON, renderIconHtml } from '../../utils/icon-utils.js';
...
export class ToolCategoriesModule extends CrudCardModule {
  constructor() {
    super({
      name: 'Categories',
      ...
      menuActions: {
        'copy-key': function copyKey(id) { this.copyCategoryKey(id); },
      },
    });
  ...
  bindEvents() {
    ...
    this.on($id('toolCatAddKey'), 'input', () => { this.addKeyTouched = true; });
  ...
  getDeleteName(record) { return record.label || record.key; }

  copyCategoryKey(id) {
    const record = this.findById(id);
    if (!record) return;
    navigator.clipboard?.writeText(record.key).then(
      () => this.toast(`Copied key "${record.key}" to clipboard.`, 'success', 2000),
  ...
  navigateToTools(categoryId) {
    try { sessionStorage.setItem('pa_tools_cat_filter', String(categoryId)); } catch { /* ignore */ }
  ...
  renderStats() {
    ...
    const techByCategory = technologies.filter((t) =>
      records.some((r) => String(r.id) === String(t.categoryId)),
    );
  ...
  validateForm(prefix) {
    ...
    if (!label) { this._setFieldError(`${p}Label`, false); valid = false; }
    else { this._setFieldError(`${p}Label`, true); }

    const pctRaw = $id(`${p}Pct`)?.value;
    const pct = Number(pctRaw);
    if (!Number.isFinite(pct) || pct < 0 || pct > 100) { this._setFieldError(`${p}Pct`, false); valid = false; }
    else { this._setFieldError(`${p}Pct`, true); }
