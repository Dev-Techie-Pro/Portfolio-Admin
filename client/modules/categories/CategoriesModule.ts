...
export class CategoriesModule extends CrudCardModule {
  ...
  navigateToProjects(key) {
    try { sessionStorage.setItem('pa_projects_cat_filter', key); } catch { /* ignore */ }
  ...
  populateCategoryFilter() {
    ...
    select.innerHTML = '<option value="all">All Categories</option>'
      + records.map((r) => `<option value="${escapeHtml(r.key)}">${escapeHtml(r.label)}</option>`).join('');
  ...
  async bulkDelete(ids) {
    ...
    if (blocked.length) {
      this.toast(`Skipped ${blocked.length} protected categor${blocked.length === 1 ? 'y' : 'ies'}: ${blocked.join(', ')}`, 'info', 4000);
    }
  }
  ...
  renderCard(record, index) {
    ...
    const builtinBadge = record.isBuiltin
      ? '<span class="pa-cat-card__builtin">Built-in</span>'
      : '';
    const bulkCheckbox = this.isCategoryDeletable(record)
      ? (this.bulkSelect?.checkboxHtml(record.key, `Select ${label}`) || '')
      : '';
  ...
  resetAddForm() {
    this.addKeyTouched = false;
    ['catAddKey', 'catAddLabel', 'catAddDesc'].forEach((id) => { const el = $id(id); if (el) el.value = ''; });
  ...
  bindEvents() {
    ...
    this.on($id('catAddKey'), 'input', () => { this.addKeyTouched = true; });
