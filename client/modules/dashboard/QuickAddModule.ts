...
export class QuickAddModule extends Module {
  ...
  populateProjectCategories() {
    ...
    if (!sel) return;
    const keys = new Set([...Object.keys(CATEGORY_META_PROJECTS), ...Object.keys(this.categoriesMap || {})]);
    const current = sel.value;
  ...
  bindEvents() {
    ...
    $id('qaPrimaryBtn')?.addEventListener('click', () => { void this.onPrimary(); });
  ...
  open(tab = 'project') {
    ...
    setTimeout(() => {
      const focusMap = {
        project: 'qaPrjTitle',
        testimonial: 'qaTestiName', experience: 'qaExpTitle', blog: 'qaBlogTitle',
      };
  ...
  validateProjectBasic() {
    ...
    if (!title) { this.showErr('qaPrjTitle', true); ok = false; } else this.showErr('qaPrjTitle', false);
    const cat = $id('qaPrjCategory')?.value;
    if (!cat) { this.showErr('qaPrjCategory', true); ok = false; } else this.showErr('qaPrjCategory', false);
    const desc = $id('qaPrjShortDesc')?.value.trim();
    if (!desc) { this.showErr('qaPrjShortDesc', true); ok = false; } else this.showErr('qaPrjShortDesc', false);
    const full = $id('qaPrjFullDesc')?.textContent.trim();
    if (!full) { $id('qaPrjFullDescError')?.classList.add('visible'); ok = false; } else $id('qaPrjFullDescError')?.classList.remove('visible');
  ...
  async submitProject() {
    ...
    if (liveUrl && !isValidUrl(liveUrl)) { this.toast('Live URL must include https://', 'danger'); return false; }
    if (repoUrl && !isValidUrl(repoUrl)) { this.toast('Repository URL must include https://', 'danger'); return false; }
    ...
    const newProject = {
      ...
      imageUrl, bannerImgUrl: imageUrl,
  ...
  async submitTestimonial() {
    ...
    let ok = true;
    if (!name) { this.showErr('qaTestiName', true); ok = false; } else this.showErr('qaTestiName', false);
    if (!quote) { this.showErr('qaTestiQuote', true); ok = false; } else this.showErr('qaTestiQuote', false);
    if (!ok) { this.toast('Please fill in all required fields', 'danger'); return false; }
  ...
  validateExpDetails() {
    let ok = true;
    if (!$id('qaExpTitle')?.value.trim()) { this.showErr('qaExpTitle', true); ok = false; } else this.showErr('qaExpTitle', false);
    if (!$id('qaExpCompany')?.value.trim()) { this.showErr('qaExpCompany', true); ok = false; } else this.showErr('qaExpCompany', false);
    if (!$id('qaExpType')?.value) { this.showErr('qaExpType', true); ok = false; } else this.showErr('qaExpType', false);
    if (!ok) this.toast('Please fill in all required fields', 'danger');
    return ok;
  }
  ...
  wireBlogForm() {
    ...
    this.on($id('qaBlogSlug'), 'input', () => { this.blogSlugTouched = true; });
  ...
  validateBlogContent() {
    let ok = true;
    if (!$id('qaBlogTitle')?.value.trim()) { this.showErr('qaBlogTitle', true); ok = false; } else this.showErr('qaBlogTitle', false);
  ...
  resetBlogForm() {
    ...
    setVal('qaBlogPublishedDate', new Date().toISOString().slice(0, 10));
  ...
  setupFeaturedUpload(boxId, inputId, getImg, setImg, folder = 'projects') {
    ...
    ['dragenter', 'dragover'].forEach((evt) => this.on(box, evt, (e) => { e.preventDefault(); box.classList.add('dragover'); }));
    ['dragleave', 'drop'].forEach((evt) => this.on(box, evt, (e) => { e.preventDefault(); box.classList.remove('dragover'); }));
