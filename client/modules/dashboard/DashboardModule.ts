...
import { CATEGORY_META_PROJECTS } from '../projects/ProjectsModule.js';
import { GROUP_META, LEVEL_META } from '../technologies/TechnologiesModule.js';
import {
  assignSegmentColors,
  assignSeriesColors,
  destroyChart,
...
const KNOWN_PROJECT_STATUSES = new Set([
  'Completed', 'In Progress', 'Pending', 'Draft', 'On Hold', 'Cancelled',
]);
...
const PROJECT_STATUS_ORDER = ['Completed', 'In Progress', 'Pending', 'Draft', 'On Hold', 'Cancelled'];

const TECH_GROUP_ORDER = ['frontend', 'backend', 'database', 'devops', 'mobile', 'design', 'other'];

const PROJECT_CATEGORY_ORDER = Object.keys(CATEGORY_META_PROJECTS);

const TECH_LEVEL_ORDER = ['expert', 'advanced', 'intermediate', 'beginner'];

const THUMB_CLASSES = ['pa-dash-thumb-orange', 'pa-dash-thumb-purple', 'pa-dash-thumb-pink', 'pa-dash-thumb-blue'];

const ACTIVITY_DOT_CLASSES = ['green', 'pink', 'blue', 'cyan'];
...
export class DashboardModule extends Module {
  constructor() {
    ...
    this.projectsYear = new Date().getFullYear();
  ...
  async load() {
    const [
      projects, technologies, experience, testimonials, media, tools, categoryMeta, recentActivities,
    ] = await Promise.all([
  ...
  bindEvents() {
    ...
    if (yearSelect) {
      this.on(yearSelect, 'change', () => {
        this.projectsYear = Number(yearSelect.value) || new Date().getFullYear();
        this.renderProjectsOverTimeLine();
      });
    }
      ...
      legend.innerHTML = coloredSegments.length ? coloredSegments.map((seg) => {
        ...
        return `<div class="pa-donut-legend-item">
          ...
          <span class="pa-donut-legend-copy">
            ...
            <span class="pa-donut-legend-meta">${seg.value} · ${pct}%</span>
          </span>
        </div>`;
  ...
  renderStatusDonut() {
    ...
    const filteredProjects = this.statusFilter === 'all'
      ? this.projects
      : this.projects.filter((project) => (project.status || 'Unknown') === this.statusFilter);
  ...
  renderTechGroupDonut() {
    ...
    const filteredTechnologies = this.techGroupFilter === 'all'
      ? this.technologies
      : this.technologies.filter((tech) => (tech.group || 'other') === this.techGroupFilter);
  ...
  renderTechLevelDonut() {
    ...
    const filteredTechnologies = this.techLevelFilter === 'all'
      ? this.technologies
      : this.technologies.filter((tech) => (tech.level || 'unrated') === this.techLevelFilter);
  ...
  renderCompletionGauge() {
    ...
    const filteredProjects = this.completionCategoryFilter === 'all'
      ? this.projects
      : this.projects.filter((project) => project.catKey === this.completionCategoryFilter);
  ...
  destroy() {
    ...
    document.querySelectorAll('.pa-donut-canvas, .pa-gauge-canvas, .pa-canvasjs-mount-line, .pa-canvasjs-mount-multiline')
      .forEach((el) => destroyChart(el));
    super.destroy();
  }
}
