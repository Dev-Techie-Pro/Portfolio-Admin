...
async function downloadTotpQr(payload = {}) {
  ...
  if (svg) {
    const blob = await renderTotpQrToPngBlob(`data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`);
...
export class SecurityManager {
  ...
  bindEvents() {
    this.wireDom();
    this.on($id('setup2faBtn'), 'click', () => { void this.startEnrollment(); });
    ...
    this.on($id('sec2faModalConfirm'), 'click', () => { void this.confirmEnrollment(); });
    this.on($id('sec2faDownloadQrBtn'), 'click', () => { void this.downloadEnrollmentQr(); });
    this.on($id('secDisableConfirm'), 'click', () => { void this.disableTwoFactor(); });
    this.on($id('secRegenBackupBtn'), 'click', () => { void this.regenerateBackupCodes(); });
    ...
    this.on($id('secDeleteConfirm'), 'click', () => { void this.deleteAccount(); });
  }
  ...
  render() {
    ...
    if (setupBtn) {
      ...
      setupBtn.innerHTML = this.state.twoFactorEnabled
        ? '<i class="ri-settings-line"></i> Reconfigure'
    ...
    if (backupWrap) {
      if (this.state.backupCodes.length) {
        backupWrap.innerHTML = this.state.backupCodes
