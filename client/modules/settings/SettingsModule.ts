...
type LoginActivityScope = 'all' | 'self';

type ApplyProfileUpdateOptions = {
  toastMessage?: string;
  notify?: boolean;
  bumpSeq?: boolean;
};
...
export class SettingsModule extends Module {
  _profileLoadSeq = 0;
  _loginActivityScope: LoginActivityScope = 'self';
  ...
  wireGeneralFormFields() {
    const panel = document.querySelector<HTMLElement>('.pa-tab-panel[data-content="general"]');
  ...
  wireNotificationFormFields() {
    const panel = document.querySelector<HTMLElement>('.pa-tab-panel[data-content="notifications"]');
  ...
  hydrateNotificationPreferences(prefs = this.store.get('notificationPreferences')) {
    ...
    document.querySelectorAll<HTMLInputElement>('input[name="notifFrequency"]').forEach((input) => {
    ...
    if (quietTimezone && prefs.quietHoursTimezone) {
      const match = [...quietTimezone.options].find((opt) => opt.text.includes(prefs.quietHoursTimezone)
        || opt.text === prefs.quietHoursTimezone);
  ...
  collectNotificationPreferences() {
    const frequencyInput = document.querySelector<HTMLInputElement>('input[name="notifFrequency"]:checked');
    const quietTimezone = $select('quietTimezone');
    return {
      ...
      quietHoursTimezone: quietTimezone?.selectedOptions?.[0]?.text
        || quietTimezone?.value
  ...
  applySettingsAccess(role) {
    ...
    const panel = document.querySelector('.pa-tab-panel[data-panel="settings"][data-content="general"]')
      || document.querySelector('#paSettingsGeneral');
    ...
    if (container && Array.isArray(p.socialLinks) && p.socialLinks.length) {
      ...
      if (first) {
        ...
        p.socialLinks.forEach((link, i) => {
          const group = first.cloneNode(true) as HTMLElement;
          const input = asFormField(group.querySelector('.pa-form-input'));
          if (input) input.value = link;
          const removeBtn = group.querySelector<HTMLElement>('.pa-social-remove');
  ...
  renderLoginActivityItem(item) {
    ...
    const detail = item.status === 'failed' && item.failureReason
      ? `${device} · ${escapeHtml(item.failureReason)}`
      : device;
    const itemClass = item.status === 'failed' ? ' pa-session-item--failed' : '';
    const userLine = this._loginActivityScope === 'all' && (item.userName || item.userEmail)
      ? `<div class="pa-session-user">${escapeHtml(item.userName || 'Staff')}${item.userEmail ? ` · ${escapeHtml(item.userEmail)}` : ''}</div>`
      : '';
  ...
  async loadLoginActivity() {
    ...
    try {
      ...
      if (!items?.length) {
        list.innerHTML = this._loginActivityScope === 'all'
          ? '<div class="pa-session-empty">No staff login activity recorded yet.</div>'
          : '<div class="pa-session-empty">No login activity yet. Sign-in attempts will appear here.</div>';
        return;
      }
  ...
  bindEvents() {
    ...
    if (saveGeneralBtn) {
      this.on(saveGeneralBtn, 'click', () => { void this.saveGeneralSettings(); });
    }

    this.wireGeneralFormFields();
    const saveOtherBtn = document.querySelector('[data-save="other"]');
    if (saveOtherBtn) {
      this.on(saveOtherBtn, 'click', () => { void this.saveOtherSettings(); });
    }

    const saveSecurityBtn = document.querySelector('[data-save="security"]');
    if (saveSecurityBtn) {
      this.on(saveSecurityBtn, 'click', () => { void this.updatePassword(); });
    }
    ...
    if (saveNotificationsBtn) {
      this.on(saveNotificationsBtn, 'click', () => { void this.saveNotificationPreferences(); });
    }
    ...
    if (container) {
      this.on(container, 'click', (e) => {
        const removeBtn = (e.target as Element).closest('.pa-social-remove');
        if (removeBtn) {
    ...
    if (updatePasswordBtn) {
      this.on(updatePasswordBtn, 'click', () => { void this.updatePassword(); });
    }
    ...
    if (logoutAllBtn) {
      this.on(logoutAllBtn, 'click', () => { void this.logoutAllDevices(); });
    }

    const refreshSessionsBtn = $id('refreshSessionsBtn');
    if (refreshSessionsBtn) {
      this.on(refreshSessionsBtn, 'click', () => { void this.loadLoginActivity(); });
    }

    const logoutAllDevicesBtn = $id('logoutAllDevicesBtn');
    if (logoutAllDevicesBtn) {
      this.on(logoutAllDevicesBtn, 'click', () => { void this.logoutAllDevices(); });
    }

    const saveQuietHours = $id('saveQuietHours');
    if (saveQuietHours) {
      this.on(saveQuietHours, 'click', () => { void this.saveNotificationPreferences({ quietOnly: true }); });
    }

    const sendTestNotification = $id('sendTestNotification');
    if (sendTestNotification) {
      this.on(sendTestNotification, 'click', () => { void this.sendTestNotification(); });
    }
  ...
  async applyProfileUpdate(profile, { toastMessage, notify = false, bumpSeq = true }: ApplyProfileUpdateOptions = {}) {
  ...
  async handleAvatarFile(file, inputEl) {
    ...
    showStatusToast('Saving avatar…', 'info', 120000);
  ...
  async handleCoverFile(file, inputEl) {
    ...
    showStatusToast('Saving cover image…', 'info', 120000);
  ...
  addSocialLink() {
    ...
    if (lastGroup) {
      const newGroup = lastGroup.cloneNode(true) as HTMLElement;
      const input = asFormField(newGroup.querySelector('.pa-form-input'));
      if (input) input.value = '';
      const removeBtn = newGroup.querySelector<HTMLElement>('.pa-social-remove');
      if (removeBtn) {
