...
export class UsersModule extends CrudCardModule {
  constructor() {
    super({
      name: 'Users',
      ...
      bulkSelect: false,
      menuActions: {
        'reset-credentials': (id) => { void this.resetCredentials(id); },
      },
    });
  ...
  async persist() {
    // API-driven page — no local persistence.
  }
  ...
  renderCard(record) {
    const initials = (record.fullName || record.email || '?')
      .split(' ')
      .map((part) => part[0])
      .slice(0, 2)
      .join('')
    ...
    const selfBadge = this.isCurrentUser(record)
      ? '<span class="pa-cat-card__builtin">You</span>'
      : '';
    ...
    return renderPaCatCard({
      ...
      desc: `@${record.username || 'user'} · ${formatRoleLabel(record.role)}`,
  ...
  confirmResetCredentials(id) {
    ...
    requestConfirm({
      ...
      iconTone: 'warning',
      danger: false,
      onConfirm: () => { void this.performResetCredentials(id); },
    });
  }
  ...
  async performResetCredentials(id) {
    ...
    this.statusToast('Resetting credentials…', 'info', 120000);
    try {
  ...
  async handleAddSubmit() {
    ...
    this.statusToast('Creating user…', 'info', 120000);
    try {
      const payload = {
  ...
  async handleEditSubmit() {
    ...
    this.statusToast('Saving changes…', 'info', 120000);
    try {
      const data = await this.fetchJson(`/api/users/${encodeURIComponent(this.currentEditId)}`, {
        method: 'PATCH',
        body: JSON.stringify(payload),
      });
      const records = this.store.get('records').map((row) => (
        String(row.id) === String(data.user.id) ? { ...row, ...data.user } : row
      ));
  ...
  async deleteById(id) {
    ...
    this.statusToast('Deleting user…', 'info', 120000);
    try {
