...
function renderPaCatCard(opts) {
  ...
  const viewBtnHtml = viewBtn ? `<button type="button" class="pa-cat-card__view-btn" ${idAttr}="${safeId}" aria-label="${escapeHtml(viewBtn.ariaLabel || viewBtn.label)}"><span>${escapeHtml(viewBtn.label)}</span><i class="ri-arrow-right-line"></i></button>` : "";
