import {
  ...
  populateChips,
  setupRte
} from "./chunk-VLIVBN4T.js";
import {
  open
} from "./chunk-UFQ6HDIL.js";
import {
...
var ProjectsModule = class extends Module {
  ...
  getFiltered() {
    ...
    if (searchQuery.trim()) {
      ...
      results = results.filter(
        (p) => p.title.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q) || p.fullDesc && p.fullDesc.toLowerCase().includes(q) || (p.tags || []).some((t) => t.toLowerCase().includes(q)) || CATEGORY_META_PROJECTS[p.catKey] && CATEGORY_META_PROJECTS[p.catKey].label.toLowerCase().includes(q)
      );
    }
    return results;
  }
    ...
    if (wrap) {
      ...
      if (removeBtn) {
        removeBtn.addEventListener("click", () => {
          if (prefix === "add") this.addFeaturedImage = null;
          else this.editFeaturedImage = null;
  ...
  renderGalleryGrid(prefix) {
    ...
    grid.querySelectorAll(".pa-gallery-thumb-remove").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (prefix === "add") this.addGalleryImages.splice(i, 1);
        else this.editGalleryImages.splice(i, 1);
  ...
  bindEvents() {
    ...
    if (addSubmit) this.on(addSubmit, "click", () => {
      void this.onAddPrimary();
    });
    ...
    if (editSubmit) this.on(editSubmit, "click", () => {
