...
// client/core/BulkSelectController.ts
var DEFAULT_IDS = {
...
var BulkSelectController = class {
  /**
  ...
  cardClass(id) {
    return this.isSelected(id) ? " pa-selected" : "";
  }
  _canSelect(id) {
  ...
  toggleSelect(id) {
    ...
    if (this.selectedIds.has(key)) {
      ...
      this.module.toast?.("This item cannot be selected for deletion.", "info", 2e3);
      return;
    } else {
      this.selectedIds.add(key);
    }
  ...
  selectAllVisible() {
    ...
    if (added > 0) {
      ...
      this.module.toast?.("No deletable items on this page.", "info", 2e3);
    }
  }
  clearSelection() {
