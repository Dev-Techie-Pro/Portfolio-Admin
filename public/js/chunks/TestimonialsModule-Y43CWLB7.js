...
var TestimonialsModule = class extends CrudCardModule {
  constructor() {
    super({
      ...
      ids: {
        grid: "paTestiGrid",
        resultCount: "paTestiResultCount",
        paginationBtns: "paTestiPaginationBtns",
        paginationInfo: "paTestiPaginationInfo",
        pagePrev: "paTestiPagePrev",
        pageNext: "paTestiPageNext",
        bodyScroll: "paTestiBody",
        emptyResetBtn: "paTestiEmptyResetBtn",
        emptyAddBtn: "paTestiEmptyAddBtn",
        addPanel: "paTestiAddPanel",
        editPanel: "paTestiEditPanel",
        addSubmit: "paTestiAddSubmit",
        editSubmit: "paTestiEditSubmit",
        editDelete: "paTestiEditDelete",
        addNewBtn: "paTestiAddNewBtn",
        addPanelClose: "paTestiAddPanelClose",
        editPanelClose: "paTestiEditPanelClose",
        addCancel: "paTestiAddCancel",
        editCancel: "paTestiEditCancel"
      },
      menuActions: { "copy-quote": function copyQuote(id) {
        this.copyQuote(id);
      } }
    });
  ...
  matchesSearch(record, query) {
    ...
    return record.name.toLowerCase().includes(q) || (record.role || "").toLowerCase().includes(q) || (record.company || "").toLowerCase().includes(q) || (record.quote || "").toLowerCase().includes(q);
  }
  renderStats() {
  ...
  renderCard(t, index) {
    ...
    const avatar = t.imageUrl ? `<img src="${escapeHtml(t.imageUrl)}" alt="${escapeHtml(t.imageAlt || t.name)}" style="width:100%;height:100%;object-fit:cover;display:block;border-radius:inherit;" />` : `<span>${escapeHtml(initials)}</span>`;
    const roleLine = [t.role, t.company].filter(Boolean).join(" \xB7 ") || "Client";
  ...
  copyQuote(id) {
    ...
    navigator.clipboard?.writeText(t.quote).then(
      () => this.toast("Copied quote to clipboard.", "success", 2e3),
      () => this.toast("Clipboard not available.", "danger")
    );
  }
  buildDuplicate(record, newId) {
    return { ...record, id: newId, name: `${record.name} (Copy)`, featured: false, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
  }
  ...
  setupAvatarDropzone(prefix) {
    ...
    const setData = (dataUrl) => {
      if (prefix === "testiAdd") this.addImageData = dataUrl;
  ...
  resetAddForm() {
    ["testiAddName", "testiAddRole", "testiAddCompany", "testiAddQuote", "testiAddAlt"].forEach((id) => {
      const el = $id(id);
      if (el) el.value = "";
    });
  ...
  validateForm(prefix) {
    ...
    if (!name) {
      $id(`${p}Name`)?.classList.add("error");
      $id(`${p}NameError`)?.classList.add("visible");
      valid = false;
    } else {
      $id(`${p}Name`)?.classList.remove("error");
      $id(`${p}NameError`)?.classList.remove("visible");
    }
    const quote = ($id(`${p}Quote`)?.value || "").trim();
    if (!quote) {
      $id(`${p}Quote`)?.classList.add("error");
      $id(`${p}QuoteError`)?.classList.add("visible");
      valid = false;
    } else {
      $id(`${p}Quote`)?.classList.remove("error");
      $id(`${p}QuoteError`)?.classList.remove("visible");
    }
