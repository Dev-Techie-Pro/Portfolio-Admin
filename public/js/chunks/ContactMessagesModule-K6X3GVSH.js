...
// client/modules/contact-messages/ContactMessagesModule.ts
var MSG_AVATAR_COLORS = ["#e5484d", "#f0c040", "#22c55e", "#38bdf8", "#a78bfa", "#f472b6", "#ff6600", "#2dd4bf"];
var PAGE_SIZE = 8;
...
var ContactMessagesModule = class extends Module {
  constructor() {
    super({
  ...
  attachReplyCardCollapseListeners(root) {
    root?.querySelectorAll(".pa-msg-reply-card").forEach((card) => {
      ...
      const setOpen = (open2) => {
        card.classList.toggle("is-open", open2);
        toggles.forEach((el) => el.setAttribute("aria-expanded", open2 ? "true" : "false"));
      };
  ...
  bindEvents() {
    ...
    if (upload && fileInput) {
      ...
      this.on(fileInput, "change", async () => {
        ...
        try {
          const { uploadContactAttachment } = await import("./media-upload-ZRQZSKPT.js");
