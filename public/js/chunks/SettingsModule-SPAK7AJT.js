...
var SettingsModule = class extends Module {
  ...
  bindEvents() {
    ...
    this.on(window, "popstate", () => {
      this.applySettingsPage(getSettingsTabFromPath());
    });
  ...
  applySettingsPage(tab) {
    ...
    document.querySelectorAll('.pa-tab-panel[data-panel="settings"]').forEach((panel) => {
      panel.classList.toggle("active", panel.dataset.content === resolved);
    });
  ...
  syncUI() {
    ...
    if (hashTab && SETTINGS_TABS.includes(hashTab)) {
      const nextPath = getSettingsTabPath(hashTab);
      window.history.replaceState({ settingsTab: hashTab }, "", nextPath);
      this.applySettingsPage(hashTab);
      return;
    }
    const path = window.location.pathname.replace(/\/$/, "") || "/";
    if (path === "/settings") {
      window.history.replaceState({ settingsTab: "general" }, "", getSettingsTabPath("general"));
      this.applySettingsPage("general");
      return;
    }
